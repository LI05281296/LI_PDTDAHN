...
for i in range(5):
    ...
    bayes_classifier.fit(np.array(X_train), Y_train_encoded)

    y_score0 = bayes_classifier.predict(np.array(X_test))
    y_score_bayes = bayes_classifier.predict_proba(np.array(X_test))

    fpr, tpr, thresholds = roc_curve(Y_test_encoded, y_score_bayes[:, 1])
    ...
    precision_, recall_, _ = precision_recall_curve(Y_test_encoded, y_score_bayes[:, 1])
    # AUPR
...
#result_df.to_excel("classification_results_bys.xlsx", index=False)
