...
for i in range(5):
    ...
    best_SVM.fit(np.array(X_train), Y_train_encoded)
    y_score0 = best_SVM.predict(np.array(X_test))
    y_score_SVM = best_SVM.predict_proba(np.array(X_test))
    fpr, tpr, thresholds = roc_curve(Y_test_encoded, y_score_SVM[:, 1])
    ...
    precision_, recall_, _ = precision_recall_curve(Y_test_encoded, y_score_SVM[:, 1])
    # AUPR
...
print(f'SVM-dimension：{dimension_dr}-{dimension_di}-{dimension_tar}-neg：{neg_flag}')
...
# result_df.to_excel("classification_results_svm1.xlsx", index=False)