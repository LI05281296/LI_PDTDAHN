...
for i in range(5):
    ...
    logistic_classifier.fit(np.array(X_train), Y_train_encoded)
    y_score0 = logistic_classifier.predict(np.array(X_test))
    y_score_logistic = logistic_classifier.predict_proba(np.array(X_test))
    fpr, tpr, thresholds = roc_curve(Y_test_encoded, y_score_logistic[:, 1])
    ...
    precision_, recall_, _ = precision_recall_curve(Y_test_encoded, y_score_logistic[:, 1])
    # AUPR
...
print(f'Logistic-dimension：{dimension_dr}-{dimension_di}-{dimension_tar}-neg：{neg_flag}')
...
result_df.to_excel("classification_results_logistic1.xlsx", index=False)
