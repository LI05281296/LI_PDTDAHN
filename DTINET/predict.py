...
def evaluate_combined_relationships(positive_negative_samples, drug_disease_res, drug_target_res, disease_target_res):
    ...
    # results_df.to_csv(f'predict_lable/label_{k+1}.csv', header=False, index=False)
...
def plot_roc_pr_curves(combined_results, k):
    ...
    for index, row in combined_results.iterrows():
        ...
        # 三个模型的结果都为正时，认为有关系
        if score_drug_disease > 0.5 and score_drug_target > 0.5 and score_disease_target > 0.5:
            prediction = 1
        else:
            prediction = 0
        y.append(prediction)
    ...
    np.savetxt(f'predict_lable/neg0/fold_{k}_roc.txt', roc_curve_data, delimiter=',', fmt='%.8f')
    ...
    np.savetxt(f'predict_lable/neg0/fold_{k}_pr.txt', pr_curve_data, delimiter=',', fmt='%.8f')
...
if __name__ == "__main__":
    ...
    positive_samples = pd.read_csv('../data_ddt/pos.csv', names=['drug', 'disease', 'target', 'label'], header=None)
    '''
    negative_samples0 = pd.read_csv('../data_ddt/neg0.csv', names=['drug', 'disease', 'target', 'label'], header=None)

    negative_samples1 = pd.read_csv('../data_ddt/neg1.csv', names=['drug', 'disease', 'target', 'label'],
                                   header=None)
    negative_samples2 = pd.read_csv('../data_ddt/neg2.csv', names=['drug', 'disease', 'target', 'label'],
                                   header=None)
    negative_samples3 = pd.read_csv('../data_ddt/neg3.csv', names=['drug', 'disease', 'target', 'label'],
                                   header=None)
    negative_samples4 = pd.read_csv('../data_ddt/neg4.csv', names=['drug', 'disease', 'target', 'label'],
                                   header=None)
    negative_samples5 = pd.read_csv('../data_ddt/neg5.csv', names=['drug', 'disease', 'target', 'label'],
                                   header=None)
    negative_samples6 = pd.read_csv('../data_ddt/neg6.csv', names=['drug', 'disease', 'target', 'label'],
                                   header=None)
    ...
    negative_samples = pd.read_csv('../data_ddt/neg6.csv', names=['drug', 'disease', 'target', 'label'], header=None)
    ...
    for k in range(5):
        drug_disease_res = pd.read_csv(f'drdi_fold{k+1}.txt', header=None, delimiter='\t')
        drug_disease_res = normalize_matrix(drug_disease_res)
        drug_target_res = pd.read_csv(f'drtar_fold{k+1}.txt', header=None, delimiter='\t')
        drug_target_res = normalize_matrix(drug_target_res)
        disease_target_res = pd.read_csv(f'ditar_fold{k+1}.txt', header=None, delimiter='\t')
        disease_target_res = normalize_matrix(disease_target_res)
